...
print "%s    / _/ / o \  %s[ __._. _ . .._ %s |__|  _. _.;_/ %s" % (W, B, R, N)
print "%s   /_/  /___,'  %s[_./[  (_)(_|[_)%s |  | (_](_.| \ %s" % (W, B, R, N)
print " %s============================%s|%s======================%s " % (C, B, C, N)
print "  %sFB%s Group%s Hack %s10-08-2017%s (0:55)%s" % (W, B, R, C, Y, N)
print "  %sAuthor: %sDedSecTL %sand%s Mr.b0t4k%s" % (W, R, W, Y, N)
print "  %sCode: %sPython%s" % (W, C, N)
print "  %sBlog: %shttp://droidsec9798-com.mwapblog.com%s" % (W, B, N)
...
if bahasa.strip() in "01 1".split():
  ...
  print "%s--------------------%s" % (R, N)
  tahap2 = raw_input("%s2%s)%s Setelah itu klik url grupnya dan salin 15 digit kode ajaib grup. Contoh punya grup %sADM alias Anak micin/alay%s (%s 589101351254979%s )%s...%s[%sEnter%s]%s" % (Y, C, W, R, C, B, C, W, C, Y, C, N))
  print "%s--------------------%s" % (R, N)
